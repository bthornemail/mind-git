...
- ✅ **Install and set up** the complete development environment
- ✅ **Create your first canvas** with observer and basic nodes
- ✅ **Understand the core concept**: Spatial arrangement = computation
- ✅ **Compile and run** your first visual program
- ✅ **Debug common issues** that beginners encounter
...
- **Why at (0,0)**: Only position that encodes to constant polynomial P₀ = 1
...
- **Flow**: Observer (1) × Hello World = Hello World
...
if (result.status === 'success') {
  ...
  console.log('✅ Code saved to hello.js');
}
"
...
{
  "id": "observer",
  "x": 50,  // ❌ WRONG! Should be 0
  "y": 50,  // ❌ WRONG! Should be 0
  "text": "#Observer:"
}
```
...
**Why it matters**: Only (0,0) encodes to polynomial P₀ = 1 (identity element).
...
```json
{
  "text": "#Invalid: something",  // ❌ UNKNOWN PREFIX
}
```
...
2. **Enable Plugin**: Settings → Community Plugins → Enable "Logos Visual Compiler"
3. **Create Canvas**: Create new file with `.canvas` extension
4. **Visual Edit**: Add nodes by typing `#` prefixes
5. **Compile**: Use command palette (Ctrl/Cmd + P) → "Compile Canvas"
