...
/**
 * MIND-GIT Export System with Org-Mode Style Tags (Simplified)
 * 
...
class ExportSystem {
 constructor() {
    this.metadataDir = path.join(path.resolve(process.cwd(), '..'), '.metadata');
  ...
  filterMetadata(metadata, rules, options) {
    console.log(`📊 Filtering ${metadata.length} components with rules:`, rules);
    ...
    const filtered = metadata.filter(item => {
      ...
      if (rules.include.length > 0) {
        ...
        if (!hasIncludeTag) {
          console.log(`  ❌ ${item.id}: Missing include tag`);
          return false;
        }
      }
      ...
      if (hasExcludeTag) {
        console.log(`  ❌ ${item.id}: Has exclude tag`);
        return false;
      }
      
      // Apply additional filters
      if (options.minCompleteness && item.development?.completeness < options.minCompleteness) {
        console.log(`  ❌ ${item.id}: Low completeness (${item.development?.completeness} < ${options.minCompleteness})`);
        return false;
      }
      
      if (options.onlyVerified && !item.mathematical?.verification?.formalVerified) {
        console.log(`  ❌ ${item.id}: Not formally verified`);
        return false;
      }
      
      if (options.layers && !options.layers.includes(item.development?.layer)) {
        console.log(`  ❌ ${item.id}: Layer not in target (${item.development?.layer} not in ${options.layers})`);
        return false;
      }
      
      console.log(`  ✅ ${item.id}: Passed all filters`);
      return true;
    });
...
`;
  ...
  generateJSONExport(metadata, options) {
    const exportData = {
  ...
  generateTypeScriptExport(metadata, options) {
  ...
  generatePDFExport(metadata, options) {
  ...
  async loadAllMetadata() {
    ...
    const content = await fsAsync.readFile(indexPath, 'utf8');
    ...
    for (const entry of indexEntries) {
      const metadataPath = path.join(this.metadataDir, `${entry.name}.jsonl`);
      if (fs.existsSync(metadataPath)) {
        try {
          const metadataContent = await fsAsync.readFile(metadataPath, 'utf8');
          fullMetadata.push(JSON.parse(metadataContent));
        } catch (error) {
          console.warn(`⚠️ Failed to load metadata for ${entry.name}: ${error.message}`);
        }
      }
    }
  ...
  saveExport(content, target, format) {
...
switch (command) {
  case 'export':
    ...
    for (let i = 0; i < args.length; i += 2) {
      if (args[i] === '--min-completeness') {
        options.minCompleteness = parseInt(args[i + 1]);
      } else if (args[i] === '--only-verified') {
        options.onlyVerified = true;
      } else if (args[i] === '--layers') {
        options.layers = args[i + 1].split(',').map(l => parseInt(l));
      }
    }
    
    exportSystem.export(target, options);
    break;
